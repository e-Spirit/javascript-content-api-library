...
export class FSXARemoteApi implements FSXAApi {
    ...
    if (filters) {
      ...
      if (locale) {
        if (typeof locale !== 'string' || !locale.includes('_')) {
          ...
          throw new NotFoundError(FSXAApiErrors.INVALID_LOCALE)
        }
  ...
  public set remotes(value: RemoteProjectConfiguration) {
    ...
    keys.forEach((key) => {
      ...
      if (!id) {
        throw new NotFoundError(FSXAApiErrors.MISSING_REMOTE_ID)
      }
      if (!locale) {
        throw new NotFoundError(FSXAApiErrors.MISSING_REMOTE_LOCALE)
      }
    })
